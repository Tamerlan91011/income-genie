...
features_description = {
    ...
    # "credit_debit_ratio": "*Соотношение кредитовых и дебетовых оборотов",
    # "balance_to_turnover": "*Доля остатков от оборотов",
    # "products_per_age": "*Интенсивность использования продуктов",
    # "debt_to_income": "*Долговая нагрузка",
    # "avg_transaction_amount": "*Средний оборот по транзакции",
}


def page():
    df_features = pd.read_csv("feature_importance.csv")
    df_features = df_features[0:20]
    
    for feature_name in df_features['feature']:
        df_features['feature'] = df_features['feature'].replace(feature_name, features_description[feature_name])
    
    st.title(":material/article_person: Что мы знаем о вашем клиенте?")

    col1, col2 = st.columns(2)
    with col1:
        st.metric("Всего признаков", 220)
        st.info(
            "Признаки закреплены за **одним** клиентом",
            icon=":material/overview:",
        )
    with col2:
        st.metric("Признаков отброшено", 66)
        st.info(
            "Признаки, которые были пусты более чем на 70%, были отброшены",
            icon=":material/delete:",
        )

    col3, col4 = st.columns(2)
    with col3:
        st.metric("Числовых признаков", 150)
        st.info(
            "Это средние значения, количественные значения, суммы и т.д.",
            icon=":material/123:",
        )

    with col4:
        st.metric("Категорий", 4)
        st.info(
            "Вау!",
            icon=":material/category:",
        )

    st.subheader("Мы составили топ-20 наиболее важных признаков")
    
    st.dataframe(df_features)
    st.bar_chart(df_features, x="feature", y="importance")
    
    with st.expander("Названия признаков остались прежними"):
