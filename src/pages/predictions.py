        ...
        if st.button("Посчитать доходы", icon=":material/psychology:"):
            with st.spinner("Гоблины думают..."):
                try:
                    ...
                    st.markdown(f"Наученные думать гоблинские модели обработали {len(df_results)} клиентов!")
                    ...
                    st.markdown("Люди с относительно большими доходами, готовые взять на себя долговые обязательства.")
                    
                    st.dataframe(vip_clients)
                    
                    st.header("А здесь мы показываем прогнозы моделей в ансамбле и по отдельности")
                    st.markdown("Это техническая информация")
                    
                    df_results = df_results.rename(columns={"prediction": "Доход, руб", "id": "Id пользователя"})
